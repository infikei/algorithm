...
pii seg_query(int node, int s, int e, int qs, int qe) {
    if (qe < s || e < qs) return { 1e9, 100000 };
    if (qs <= s && e <= qe) return seg[node];

    int mid = (s + e) >> 1;
    pii left_val = seg_query(node * 2, s, mid, qs, qe);
    pii right_val = seg_query(node * 2 + 1, mid + 1, e, qs, qe);
...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (cmd == 1) {
            ...
            int qs, qe;
            cin >> qs >> qe;
            cout << seg_query(1, 1, n, qs, qe).second << '\n';
        }
    }

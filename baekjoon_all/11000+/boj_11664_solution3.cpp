...
int main() {
    ...
    if (calc_inner_product(b - a, c - a) <= 0) {
        ans_dist = calc_dist(a, c);
    }
    else if (calc_inner_product(a - b, c - b) <= 0) {
        ans_dist = calc_dist(b, c);
    }
    else {
        ans_dist = calc_area_of_triangle(b - a, c - a) * 2 / calc_dist(a, b);
    }
